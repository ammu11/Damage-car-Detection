...
class ObjectDetectorLite(ObjectDetectorDetectionAPI):
    def __init__(self, model_path='models/updated_damage_size_300.tflite'):
        """
...
def main(file):
	...
	res1=[]
	image = cv2.cvtColor(cv2.imread("out/"+file), cv2.COLOR_BGR2RGB)
	...
	cv2.imwrite('out/'+file.split()[0]+"(1).jpg", cv2.cvtColor(image, cv2.COLOR_RGB2BGR))
